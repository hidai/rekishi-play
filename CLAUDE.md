...
ガードレール（**単一ゲート原則**）: このリポジトリは無人の自律開発ループ（`/auto-dev` を `/loop` 等で回す）で運用する。**ループの途中に人間の判断は入らない。** 人間が判断を下す唯一のポイントは **main に積まれた commit 群を upstream に push（＝GitHub Pages へ自動デプロイ）するか否か**であり、人間はそこでまとめてレビューして受否を決める。ゆえに AI は「人間の判断待ち」でループを止めず、自律に決めて commit まで進める（push はしない）。
