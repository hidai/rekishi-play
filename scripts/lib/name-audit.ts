...
export const RENAMED_NAMES: Record<string, string[][]> = {
  ...
  katsu: [['麟太郎', '海舟']],
  // 位が変わると呼び名も変わる人（天皇→院→法皇）。小5・中1が「3章では後白河院なので
  // 別人に見える」「病院の院かと思った」と独立に報告した＝主人公以外でも同じ型。
  // 「後白河法皇」は本文を「院」へ統一した時点でどの面にも無い＝この鎖の3つ目は
  // **前向きの見張り**（将来また法皇で書いたら結び直しを要求する）。腐りではない。
  kiyomori: [['後白河天皇', '後白河院', '後白河法皇']],
  masako: [],
  davinci: [],
  shibusawa: [],
};
