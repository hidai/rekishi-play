...
export const INSTITUTION_TERMS: string[] = [
  ...
  // 朝廷（kiyomori・masako）
  '朝廷', '天皇', '上皇', '法皇', '院', '院政', '公家', '貴族', '関白', '太政大臣', '太閤', '摂政', '朝敵', '官位',
  '令旨',
  // 位・神号・代理を名ざす層（2026-08-03 shibusawa の読み通しで小5・中1が独立に挙げた／ieyasu の回に
  // 読み通しと小5が独立に指した）。「読めるのに意味が届かない」の典型で、リストに無いあいだ計器は
  // 緑を報告し続けていた。「大臣」は現代語として通る＋太政大臣と二重に当たるので入れない。
  '爵位', '公爵', '伯爵', '子爵', '男爵', '華族', '大御所', '明神', '権現', '名代', '内務大臣',
  // 近代（katsu・shibusawa）
...
export const ALLOWED_INSTITUTION: Record<string, string> = {
  // hidenaga（2026-08-01 棚卸し・entry と ch1 ／ 2026-08-03 に残る全章）
  'hidenaga|太閤':
    '初出は 史料名『太閤素生記』の中の字＝制度語の使用ではなく本の題（固有名詞）。同じ面が「江戸時代に 書かれた 伝記」と その本の性格を言い、語としての「太閤」は主線に一度も出ない（ch2 の『絵本太閤記』も同じ）',
  'hidenaga|大名':
    '初出（6-a）の直前の一句が言い換え＝「大和・紀伊・和泉——三つの 国を あわせて 治める 約100万石の 大大名」（ieyasu|大名 と同型）。読者順で先に来る ch5 teaser の「100万石の 大大名へ」は 語を降ろした',
  ...
  'masako|天皇': '初出（5-b）が 上皇 の言い換え文の中＝「上皇とは、天皇の 位を ゆずった あとも 力を もつ 人の こと」。位＝人が就いて降りるもの、という関係が同じ一文で渡る（kiyomori|天皇 と同型）',
  ...
  'shibusawa|天皇':
    '初出（3-d）が「京に いる 天皇——代々の 将軍を 将軍に して きた 人」＝ダッシュの同格が言い換え。この作品の読者は 将軍 を先に知っているので、天皇 は将軍との関係で位置づけるのがいちばん短い（「返した」がなぜ返すと言えるのかも同じ一句で埋まる）',
  'shibusawa|子爵':
    '初出（7-a）の直後の一文が言い換え＝「もう「百姓の くせに」とは 言われない」。位の高さを定義でなく**結果**で渡す（章一 1-a で読者自身が笑われた場面の呼び返し）。「国が あたえる 位」と定義文を置く案は、爵位の由来（勲功と旧家柄）を一文で言い切れず、無い規定を作る側に倒れた',
  ...
  'kiyomori|天皇': '初出（1-b）が 朝廷 の言い換え括弧の中＝「朝廷（天皇を 中心に 国を 動かす ところ）」。語の位置づけ（国を動かす中心にいる人）がその括弧で渡る',
};
