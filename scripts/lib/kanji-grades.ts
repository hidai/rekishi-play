// 学年別漢字配当表（2017 告示・2020 施行の現行版）。小学校で習う 1,026 字を学年ごとに持つ。
//
// なぜ同梱するか: 本作品群の読者は小5・中1で、本文は <ruby> ふりがな付き HTML で書く契約
// （src/engine/types.ts の TRUST BOUNDARY）。どの字にふりがなが要るかは「その子がまだ習って
// いない字か」で決まる——つまり配当表が唯一の客観的な基準。人手（LLM 監査）で毎章見ていた
// あいだ、G8（ルビ漏れ）は章三〜章六で連続して最大の指摘だった＝人手では毎章漏れる。
//
// 出典: 文部科学省「小学校学習指導要領（平成29年告示）」別表 学年別漢字配当表。
// 取り込みは ja.wikipedia「学年別漢字配当表」の逐字リストから機械抽出し、
// 各学年の字数（80/160/200/202/193/191＝計 1,026）が告示と一致することを
// tests/ruby-furigana.test.ts が毎回検算する（表そのものが壊れたら落ちる）。

/** 添字 0 が第1学年。各要素はその学年の配当漢字を並べた文字列。 */
export const KYOIKU_KANJI_BY_GRADE: readonly string[] = [
...
KYOIKU_KANJI_BY_GRADE.forEach((chars, i) => {
  for (const ch of chars) GRADE_OF.set(ch, i + 1);
});

/** 小学校で習う学年（1〜6）。中学配当・表外字は 0。 */
export function kanjiGrade(ch: string): number {
  return GRADE_OF.get(ch) ?? 0;
}

/** CJK 統合漢字（作品データに出るのはこの範囲＋々などの記号だけ）。 */
