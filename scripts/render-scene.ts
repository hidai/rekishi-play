...
// --w=<px> は絵が敷かれる幅（既定＝設計幅 680）。読み解き地図はこの幅で窓の切り取り量が変わる
// ——携帯の見えかたは --w=358 で書き出して確かめる（map/sceneMap の f）。
// 使い方: npx vite-node scripts/render-scene.ts <出力ファイル.svg> [章] [シーンid] [work-slug=hidenaga] [--raw] [--w=px]
// 例:     npx vite-node scripts/render-scene.ts /tmp/scene.svg 7 7-a2
//         npx vite-node scripts/render-scene.ts /tmp/scene.svg 3 3-a katsu --w=358
...
const displayW = widthArg ? Number(widthArg.slice(4)) : undefined;
const argv = process.argv.slice(2).filter((a) => a !== '--raw' && !a.startsWith('--w='));
const out = argv[0];
...
let svg = buildMainVisual(work, ch, scene, s, displayW);
