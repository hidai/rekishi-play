...
// 2026-08-02 までどの計器も見ていなかった。
//
// ラチェットを持たない（BASELINE 無し）＝**全作 0 件が要求**。型4 の対象が2作しかなく、
// 同じサイクルで直しきれたから。新しい作品の主人公が改名するなら、RENAMED_PEOPLE に
// 名を足してから書く＝最初から守られる。
//
...
describe('name-continuity: 名が変わる人は、その場で前の名と結ぶ', () => {
  ...
  it('RENAMED_PEOPLE に全作品の欄がある（新作品は空配列で「改名なし」と明示する）', () => {
    for (const work of ALL_WORKS) expect(RENAMED_PEOPLE[work.id], work.id).toBeDefined();
  });
  ...
  it('名を結ばずに呼び替えると赤くなる', () => {
    ...
    // 入口が「海舟」を先に置くので、結びを外した 1-a の「麟太郎」が別人として立つ。
    expect(auditWork(broken).map((b) => `${b.surface} ${b.name}`)).toContain('ch1/1-a 麟太郎');
  });
});
