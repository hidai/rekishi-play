...
const BASELINE: Record<string, number> = {
  ...
  //   ＝ch6 は「天皇から いただいた 位は 権大納言」が裸〔天皇 の追加で露出・2026-08-03〕
  'hidenaga:ch6': 1,
  ...
  //   走査面を足して reveal に出た1件も 0 に〔2026-08-03〕＝「東照大権現」の caption の 朝廷 は
  //   この作品に一度も出ない役所。初版は「京の 天皇」に替えたが、それは**計器に載っていない
  //   同族語への乗り換え**で、3軸が独立に「決めたのは僧たち／おくったのは天皇」の主語のねじれを
  //   指した＝作品がすでに使う2つの地名へ降ろした（「えらんだのは 江戸、おくったのは 京」）。
  //   天皇 は この回に INSTITUTION_TERMS へ追加（＝乗り換え先を計器に載せた）
  //   ＝ch6「大御所」・ch7「明神／権現」は 位・神号の層を計器に足して露出した〔2026-08-03〕。
  //   7-b は この2語の差を争う画面なのに差が本文に無い＝次の ieyasu の回で棚卸しする
  'ieyasu:ch6': 1,
  'ieyasu:ch7': 2,
  ...
  //   ＝ch3 は「国を おさめる 力を 天皇に 返した」が裸〔天皇 の追加で露出・2026-08-03〕
  //   → 全7章の読み通し検査の回に 0 へ（3-d の 天皇 は将軍との関係で同格・3-a の 名代 は「かわり」へ
  //   降ろす・7-a の 子爵 は 1-a の呼び返しで・7-c の 内務大臣 は「くらしの 大臣」へ降ろす）
};
