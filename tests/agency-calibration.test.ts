...
describe('型9 の計器: 読み通しで観察された画面に合っている', () => {
  ...
  it('較正元（秀長）の「章まるごと空白」は1章まで', () => {
    // ⚠️ この線は 2026-08-04 の code review で引き直した。初版は「空白ゼロ」を要求して緑だったが、
    // **その緑は誤検出1件（3-a の「手に入れ」＝城を得た結果を行為として数えていた）だけで
    // 支えられていた**。誤検出を消すと 秀長 ch3（但馬・播磨＝土地を治める章）は本当に空白で、
    // これは計器の誤りではなく秀長に残っている型9 の実例（＝直す待ち行列に入れた）。
    // 線を「1章まで」にしたのは、2章目が空白になったら計器か作品のどちらかが動いた印だから。
    const blank = auditWork(hidenaga).filter((ch) => ch.engaged === 0);
    expect(blank.map((ch) => ch.chapterId)).toEqual([3]);
  });
