...
const LOAD_LEDGER: Record<string, number> = {
  // kiyomori: 5章（最重 7-a2 489）。ch5 は 571→414 で削除（5-c を分割・2026-08-02）、
  //   呼び名を「院」に統一して 407 へ（2026-08-03。同じ回に ch2 も 466→450）。
  //   ch3 は 454→420 で削除（3-c2 を足すため章の説明を刈った・2026-08-04）。
  'kiyomori:1': 483, 'kiyomori:2': 446, 'kiyomori:4': 431,
  'kiyomori:6': 426, 'kiyomori:7': 489,
  // ieyasu: 1章（最重 3-b 464）。ch1 は 528→364 で削除（1-c を分割・2026-08-02）。
  //   ch4 は 562→407 で削除（4-b を分割し、露出した 4-d も刈った・2026-08-05）。
  //   ch5 は 541→381 で削除（5-b を分割・2026-08-05）。
  //   ch6 は 510→411 で削除（6-b を分割し、露出した 6-d の信条も刈った・2026-08-05）。
  'ieyasu:3': 464,
  // masako: 4章
